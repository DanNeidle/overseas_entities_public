...
def precompute_entity_counts(cursor: sqlite3.Cursor) -> tuple[dict, dict]:
    ...
    bo_columns = [f"proprietor{p}_BO{bo}_name" for p in range(1, 5) for bo in range(1, 5)]
    
    # The subquery now just collects the raw names
    union_all_bo_query = " UNION ALL ".join(
        f"SELECT {col} AS name FROM entities WHERE {col} IS NOT NULL" for col in bo_columns
    )
...
def extract_data(current_db_file, output_json_path):
    ...
    for index, database_row in enumerate(rows):
        ...
        for p in range(1, 5):
            ...
            if pname:
                ...
                if contains_trust_keyword(pname):
                ...
                elif (p_at_least_one_trustee_controller and not p_at_least_one_non_trustee_controller) or (contains_trust_keyword(pname) and not p_at_least_one_non_trustee_controller):
    ...
    # Write split JSONs: properties index + proprietors dictionary with cache-busting hash
    properties_short, proprietors_short = split_and_compress(properties)

    # Serialize to bytes (consistent formatting) for hashing
    props_bytes = json.dumps(properties_short, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    owners_bytes = json.dumps(proprietors_short, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    combined_bytes = props_bytes + b"\n" + owners_bytes
    sha = hashlib.sha256(combined_bytes).hexdigest()[:8]

    # Clean up old hashed JSONs and manifest
    for pattern in (
        str(WEBAPP_DIR / f"{PROPERTIES_JSON_BASENAME}.*.json"),
        str(WEBAPP_DIR / f"{PROPRIETORS_JSON_BASENAME}.*.json"),
    ):
        for path in glob.glob(pattern):
            try:
                ...
                pass
    try:
        (WEBAPP_DIR / "overseas_entities_json_info.txt").unlink()
    except Exception:
        pass

    # Write pretty JSONs to hashed filenames
    props_path = WEBAPP_DIR / f"{PROPERTIES_JSON_BASENAME}.{sha}.json"
    owners_path = WEBAPP_DIR / f"{PROPRIETORS_JSON_BASENAME}.{sha}.json"
    with open(props_path, "w", encoding="utf-8") as f:
        json.dump(properties_short, f, ensure_ascii=False, indent=2)
    with open(owners_path, "w", encoding="utf-8") as f:
        json.dump(proprietors_short, f, ensure_ascii=False, indent=2)

    # Report combined uncompressed size and write manifest (size on line1, hash on line2)
    props_size = props_path.stat().st_size if props_path.exists() else 0
    owners_size = owners_path.stat().st_size if owners_path.exists() else 0
    combined_size = props_size + owners_size
    info_txt_path = WEBAPP_DIR / "overseas_entities_json_info.txt"
    info_lines = [str(combined_size), sha]
    dataset_period = determine_dataset_period(current_overseas_entity_list_file)
    if dataset_period:
        info_lines.append(dataset_period)
    info_txt_path.write_text("\n".join(info_lines) + "\n", encoding="utf-8")

    print(f"\nData exported to {props_path} and {owners_path}")
    print(f"Combined JSON size: {combined_size:,} bytes (properties: {props_size:,}, proprietors: {owners_size:,}); hash: {sha}")
