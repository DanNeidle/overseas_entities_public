...
def get_company_details(company_name: str):
    ...
    while True:
        ...
        if r.status_code == 200:
            ...
            # print("Reached end of results (416). Stopping pagination.")
            break
        ...
        for prev_name in previous_names_in_snippet:
            ...
            if norm_query == normalise_text(prev_name.strip()):
                ...
                if item.get('company_type') in allowed_types:
                    ...
                    # Debug info to make it clear why a snippet match was ignored
                    print(
...
# Debug block removed to allow normal processing
...
def get_BO_list(company_number: str) -> dict:
    ...
    while True:
        throttle_requests()
        try:
            # print(f"DEBUG: Requesting PSCs for {company_number}")
            r = requests.get(url, auth=(companies_house_api_key, ''), timeout=30)
            if r.status_code == 200:
                return r.json()
            elif r.status_code in (404, 403):
    ...
    for row_tuple in rows:
        ...
        for i in range(1, 5):
            ...
            if prop and prop.strip():
                # print(f"DEBUG: Looking up BOs for proprietor{i}: '{prop}'")
                status, bos, reg_address, comp_number = find_BOs_for_company(prop)
                # print(f"DEBUG: Status={status}, count={len(bos)}")
                ...
                if status == 'OK':
                    ...
                    for j, bo in enumerate(bos[:4], start=1):
                        base = f'proprietor{i}_BO{j}_'
        ...
        # for debugging - print after we filter update for non-null values
        # visible = {k: v for k, v in update.items() if v is not None}
        # print('About to write update:', json.dumps(visible, indent=2))
