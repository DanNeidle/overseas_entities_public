...
def load_yearly(lines: List[str]) -> Tuple[List[str], Dict[str, Dict[str, int]]]:
    """Load the yearly table.

    Returns (years, data_by_category) where data_by_category[category][year] = count.
    """
...
def make_yearly_chart(years: List[str], data_by_category: Dict[str, Dict[str, int]]) -> Dict[str, Any]:
    ...
    overall_total = max(sum(totals.get(y, 0) for y in years), 1)
    ...
    x_labels = sorted_years + ["Total proprietors"]
    ...
    for cat in categories:
        ...
        overall_abs_cat = sum(abs_vals)
        overall_pct_cat = (overall_abs_cat / overall_total * 100.0) if overall_total else 0.0

        values: List[float] = []
        tooltip_html: List[str] = []
        for lbl in x_labels:
            if lbl == "Total proprietors":
...
def main() -> None:
    ...
    # Yearly chart
    years, yearly_data = load_yearly(sections.get("yearly", []))
    yearly_option = make_yearly_chart(years, yearly_data)
    save_option(
        yearly_option,
